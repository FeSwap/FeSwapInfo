...
const UPDATE_ALL_PAIRS_IN_FESWAP = 'UPDATE_ALL_PAIRS_IN_FESWAP'
const UPDATE_ALL_TOKENS_IN_FESWAP = 'UPDATE_ALL_TOKENS_IN_FESWAP'
...
function reducer(state, { type, payload }) {
  switch (type) {
    ...
    case UPDATE_ALL_PAIRS_IN_FESWAP: {
    ...
    case UPDATE_ALL_TOKENS_IN_FESWAP: {
...
export default function Provider({ children }) {
  ...
  const updateAllPairsInFeSwap = useCallback((allPairs) => {
    dispatch({
      type: UPDATE_ALL_PAIRS_IN_FESWAP,
      payload: {
        allPairs,
      },
    })
  ...
  const updateAllTokensInFeSwap = useCallback((allTokens) => {
    dispatch({
      type: UPDATE_ALL_TOKENS_IN_FESWAP,
      payload: {
        allTokens,
      },
    })
      ...
      value={useMemo(
        () => [
          state,
          {
            ...
            updateEthPrice,
            updateTopLps,
            updateAllPairsInFeSwap,
            updateAllTokensInFeSwap,
          },
        ],
        [
          state,
...
async function getGlobalData(ethPrice, oldEthPrice) {
  ...
  try {
    ...
    data = result.data.feSwapFactories[0]
    ...
    oneDayData = oneDayResult.data.feSwapFactories[0]

    let twoDayResult = await client.query({
      query: GLOBAL_DATA(twoDayBlock?.number),
      fetchPolicy: 'cache-first',
    })
    twoDayData = twoDayResult.data.feSwapFactories[0]

    let oneWeekResult = await client.query({
      query: GLOBAL_DATA(oneWeekBlock?.number),
      fetchPolicy: 'cache-first',
    })
    const oneWeekData = oneWeekResult.data.feSwapFactories[0]

    let twoWeekResult = await client.query({
      query: GLOBAL_DATA(twoWeekBlock?.number),
      fetchPolicy: 'cache-first',
    })
    const twoWeekData = twoWeekResult.data.feSwapFactories[0]
...
const getChartData = async (oldestDateToFetch, offsetData) => {
  ...
  try {
    while (!allFound) {
      ...
      skip += 1000
      data = data.concat(result.data.feSwapDayDatas)
      if (result.data.feSwapDayDatas.length < 1000) {
        allFound = true
      }
    }
...
/**
 * Loop through every pair on FeSwap, used for search
 */
async function getAllPairsOnFeSwap() {
  try {
    let allFound = false
    let pairs = []
    let skipCount = 0
...
/**
 * Loop through every token on FeSwap, used for search
 */
async function getAllTokensOnFeSwap() {
  try {
    let allFound = false
    let skipCount = 0
    let tokens = []
...
export function useGlobalData() {
  const [state, { update, updateAllPairsInFeSwap, updateAllTokensInFeSwap }] = useGlobalDataContext()
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      let allPairs = await getAllPairsOnFeSwap()
      updateAllPairsInFeSwap(allPairs)

      let allTokens = await getAllTokensOnFeSwap()
      updateAllTokensInFeSwap(allTokens)
    }
    if (!data && ethPrice && oldEthPrice) {
      fetchData()
    }
  }, [ethPrice, oldEthPrice, update, data, updateAllPairsInFeSwap, updateAllTokensInFeSwap])
...
export function useAllPairsInFeSwap() {
...
export function useAllTokensInFeSwap() {
  const [state] = useGlobalDataContext()
